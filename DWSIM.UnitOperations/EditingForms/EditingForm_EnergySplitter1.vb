...
Public Class EditingForm_EnergySplitter1

    Inherits SharedClasses.ObjectEditorForm

    Public Property SimObject As UnitOperations.EnergySplitter
    ...
    Sub UpdateInfo()
        ...
        With SimObject
            ...
            Select Case .OperationMode
                Case UnitOperations.EnergySplitter.OpMode.SplitRatios
                ...
                Case UnitOperations.EnergySplitter.OpMode.StreamEnergySpec
                    cbCalcMode.SelectedIndex = 1
                    cbFlowSpec1.Items.Clear()
                    cbFlowSpec1.Items.AddRange(units.GetUnitSet(Interfaces.Enums.UnitOfMeasure.heatflow).ToArray)
                    cbFlowSpec1.SelectedItem = units.massflow

                    cbFlowSpec2.Items.Clear()
                    cbFlowSpec2.Items.AddRange(units.GetUnitSet(Interfaces.Enums.UnitOfMeasure.heatflow).ToArray)
                    cbFlowSpec2.SelectedItem = units.massflow

                    tbFlowSpec1.Text = su.Converter.ConvertFromSI(units.heatflow, .StreamEnergySpec).ToString(nf)
                    tbFlowSpec2.Text = su.Converter.ConvertFromSI(units.heatflow, .Stream2EnergySpec).ToString(nf)
    ...
    Private Sub cbCalcMode_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbCalcMode.SelectedIndexChanged
        ...
        Select Case cbCalcMode.SelectedIndex
            Case 0
                ...
                cbFlowSpec1.Enabled = False
                cbFlowSpec2.Enabled = False
                SimObject.OperationMode = UnitOperations.EnergySplitter.OpMode.SplitRatios
            Case 1
                TrackBar1.Enabled = False
                TrackBar2.Enabled = False
                tbRatio1.Enabled = False
                ...
                cbFlowSpec1.Items.Clear()
                cbFlowSpec1.Items.AddRange(units.GetUnitSet(Interfaces.Enums.UnitOfMeasure.heatflow).ToArray)
                cbFlowSpec1.SelectedItem = units.heatflow
                cbFlowSpec2.Items.Clear()
                cbFlowSpec2.Items.AddRange(units.GetUnitSet(Interfaces.Enums.UnitOfMeasure.heatflow).ToArray)
                cbFlowSpec2.SelectedItem = units.heatflow
    ...
    Private Sub cb_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbFlowSpec1.SelectedIndexChanged, cbFlowSpec2.SelectedIndexChanged
        ...
        If Loaded Then
            Try
                Dim sunits = ""
                If SimObject.OperationMode = UnitOperations.EnergySplitter.OpMode.StreamEnergySpec Then
                    sunits = units.heatflow
                Else
                    sunits = units.heatflow
                End If
    ...
    Sub UpdateProps(sender As Object)
        ...
        Select Case cbCalcMode.SelectedIndex
            Case 0
                uobj.OperationMode = UnitOperations.EnergySplitter.OpMode.SplitRatios
            Case 1
                uobj.OperationMode = UnitOperations.EnergySplitter.OpMode.StreamEnergySpec
        End Select
